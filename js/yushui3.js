...
var bgColor = "rgb(40,40,40)";//改变主题颜色
var ready;
var enemyScore;
...
function init(){
    ...
    //Spawn player
    player = new Player();
    player.position.set(canvas.width,canvas.height-player.size);
    player.render();
...
//Draw background
function drawBG(){
    ctx.fillStyle = "#FE2A20";//游戏进行时的背景颜色
...
function drawStatic(alpha){
    ...
    for (var x = 0; x < canvas.width; x+=s){
        for (var y = 0; y < canvas.width; y+=s){
            ...
            ctx.fillStyle = "rgba(" + n + "," + n + "," + n + "," + (Math.random() * alpha) + ")";//改变静态加载背景
            // ctx.fillStyle="#FE2A20";
            ctx.fillRect(x,y,s,s);//fillRect(x,y,width,height);矩形左上角的 x 坐标y	矩形左上角的 y 坐标 width矩形的宽度 height矩形的高度
            // ctx.fillRect("#FE2A20");
...
function spawnEnemy(){
    ...
    var r = Math.random();//随机选择下落的方式
    if (r > .5){
        straightDownMotion(e,v);//z直落
    }
    else if (r > .3){
        sineMotion(e,a,f,v);
    }
